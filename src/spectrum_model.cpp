...
SpectrumModel *SpectrumModel::calculate(std::map<std::string, Eigen::ArrayX<double>> parameters, bool withNoise) {
    ...
    for (itr = parameters.begin(); itr != parameters.end(); ++itr) {
        if (itr->first == "Gaussian"){
            ...
            internalSpectrum += gauss.Gaussian();
        }
            ...
            internalSpectrum += lorentz.Lorenzt();
        }
        else
            internalSpectrum.setZero();
    }
    ...
    NormalDistributions.push_back(x0);
    NormalDistributions.push_back(fwhm0);
    NormalDistributions.push_back(int0);
    NormalDistributions.push_back(x1);
    NormalDistributions.push_back(fwhm1);
    NormalDistributions.push_back(int1);

    this->spectrum = internalSpectrum;
